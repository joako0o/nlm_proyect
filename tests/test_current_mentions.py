...
class CurrentMentionTests(unittest.TestCase):
    ...
    def test_thirty_nine_legitimate_mentions_and_four_pending_readings_are_documented(self):
        self.assertEqual({e['ID_Padre'] for e in self.reviews.values() if e["Decision"]=="MENCION_LEGITIMA_REVISADA"}, {320,1336,1420,2228,2325,2768,3147,4574,4656,5658,6587,571,647,1114,1328,1338,1574,2397,2430,854,1621,1926,4161,1549,2219,77,1042,1456,2167,1004,5276,5308,870,6601,2839,3124,6394,1142,1370})
        self.assertEqual(len(self.reviews), 43)
    ...
    def test_valid_annotations_do_not_modify_rows_or_warnings(self):
        ...
        self.assertEqual(len(annotations), 43)
        ...
        for values in annotations.values():
            ...
            if '-L28-' in values[FIELDS[1]] or '-L29-' in values[FIELDS[1]]:
