...
class CurrentMentionTests(unittest.TestCase):
    ...
    def test_ninety_eight_legitimate_mentions_and_four_pending_readings_are_documented(self):
        self.assertEqual({e['ID_Padre'] for e in self.reviews.values() if e["Decision"]=="MENCION_LEGITIMA_REVISADA"}, {320,1336,1420,2228,2325,2768,3147,4574,4656,5658,6587,571,647,1114,1328,1338,1574,2397,2430,854,1621,1926,4161,1549,2219,77,1042,1456,2167,1004,5276,5308,870,6601,2839,3124,6394,1142,1370,649,1298,1379,1930,1941,2058,2201,2206,2220,2853,3570,4193,4910,4989,202,410,953,1273,2499,2564,2608,2701,2743,3059,3286,3615,5078,6353,6402,6669,6891,6935,7124,7175,79,81,95,511,578,611,628,665,666,681,689,695,749,3102,3362,3473,3536,3655,4153,5578,6156,6250,6456,6533,7096})
        self.assertEqual(len(self.reviews), 102)
    ...
    def test_valid_annotations_do_not_modify_rows_or_warnings(self):
        ...
        self.assertEqual(len(annotations), 102)
        ...
        for values in annotations.values():
            ...
            if '-L28-' in values[FIELDS[1]] or '-L29-' in values[FIELDS[1]] or '-L30-' in values[FIELDS[1]] or '-L31-' in values[FIELDS[1]] or '-L32-' in values[FIELDS[1]]:
