...
class LoopTwentyOneTests(unittest.TestCase):
    ...
    def test_warning_scopes_are_exact_and_old_mentions_remain(self):
        ...
        ms=load_mention_reviews(self.raw);self.assertEqual(len(ms),38)
        self.assertEqual([e['ID_Padre'] for e in ms.values() if e['Decision']==PENDING_DECISION],[6185,3775,4055,2510])
