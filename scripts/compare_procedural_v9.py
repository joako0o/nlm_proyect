"""Gate v9: los cinco cortes del lote10 sobre la entrega procedimental v7.
...
filas. v9 agrega dos, legítimamente, así que necesita su propio gate en vez de
reescribir el de v8, que a su vez está fijado por hash en la procedencia de esa entrega.
...
PADRES = (657, 1564, 1706, 1995, 2960)
FILAS_V7 = 9723
...
def compare(before, after, revisiones):
    ...
    nuevas = [f['ID_Intervencion'] for f in after if f['ID_Intervencion'] not in id_v7]
    if len(nuevas) != 2:
        raise ValueError(f'Se esperaban 2 filas nuevas y hay {len(nuevas)}: {nuevas}')
    if [f['ID'] for f in after] != list(range(1, len(after) + 1)):
        raise ValueError('El ID dejó de ser una secuencia contigua desde 1')
    ids_nuevos = sorted(f['ID'] for f in after if f['ID_Intervencion'] not in id_v7)
    corridas = []
    for fila in after:
    ...
    if len(extras) != 2:
        raise ValueError(f'se esperaban 2 grupos nuevos y hay {len(extras)}: {extras}')
    ...
    return dict(Perfil='procedimental-v9', Baseline='procedimental-v7', Pasa=True,
...
def main():
    ...
    (args.candidate / 'comparacion_procedimental_v9.json').write_text(
