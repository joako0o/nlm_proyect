"""Construye la entrega v11 = v10 + la fusión del padre 3062 (corte al revés), y publica sólo si pasa todo.

Uso: python scripts/preparar_data_v11.py --destino data/releases/continuidad_procedimental_v11
...
v8 y v9 tienen cada uno su propio driver y su propio gate, porque el de v8
está fijado por hash en la procedencia de esa entrega y espera exactamente una
fila nueva.
...
``comparacion_procedimental_v9.json``.
"""
import argparse
import hashlib
import json
...
GATE = 'scripts/compare_procedural_v11.py'
SALIDAS_EXTRAS = ('comparacion_procedimental_v11.json', 'linaje_cortes_lote10.csv')
...
def main(argv=None):
    ...
    parser.add_argument('--destino', type=Path,
                        default=ROOT / 'data/releases/continuidad_procedimental_v11')
    args = parser.parse_args(argv)
    objetivo = destino_versionado(args.destino)

    cache = ROOT / '.cache'
    cache.mkdir(exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='preparacion-v11-', dir=cache) as temp:
        stage = Path(temp)
        ...
        comandos = [
            ...
            ['scripts/qa_preparacion_v11.py', '--processed', str(stage)],
        ]
    ...
    comparacion = json.loads((objetivo / 'comparacion_procedimental_v11.json')
