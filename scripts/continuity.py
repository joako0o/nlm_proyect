...
def annotate_turns(rows, reviewed_links=None):
    ...
    for row in rows:
        ...
        if reviewed and (not same or institutional or prev.get('Tipo_Acta')
                ...
                or 'POSIBLE_OTRO_HABLANTE_O_MENCION' in (prev.get('Motivos_Revision') or '')
                or 'POSIBLE_OTRO_HABLANTE_O_MENCION' in (row.get('Motivos_Revision') or '')):
            raise ValueError('Continuidad revisada atraviesa barrera')
        joined = bool(reviewed or (same and not blocked and previous_anchor and (source in EXPLICIT | CONTINUED or same_physical_block)))
        if joined:
            turn = prev['ID_Turno']
