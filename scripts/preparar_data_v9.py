"""Construye la entrega v9 = v7 + los cinco cortes del lote10, y publica sólo si pasa todo.

Uso: python scripts/preparar_data_v9.py --destino data/releases/continuidad_procedimental_v9
...
GATE = 'scripts/compare_procedural_v9.py'
SALIDAS_EXTRAS = ('comparacion_procedimental_v9.json', 'linaje_cortes_lote10.csv')
...
def main(argv=None):
    ...
    parser.add_argument('--destino', type=Path,
                        default=ROOT / 'data/releases/continuidad_procedimental_v9')
    args = parser.parse_args(argv)
    objetivo = destino_versionado(args.destino)

    cache = ROOT / '.cache'
    cache.mkdir(exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='preparacion-v9-', dir=cache) as temp:
        stage = Path(temp)
        ...
        comandos = [
            ...
            ['scripts/qa_preparacion_v9.py', '--processed', str(stage)],
        ]
    ...
    comparacion = json.loads((objetivo / 'comparacion_procedimental_v9.json')
