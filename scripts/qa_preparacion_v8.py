"""QA de preparación para v8: el mismo módulo, con la comparación interna apuntando a v8.
...
porque eso es lo que v7 debía probar. v8 agrega una fila legítimamente, así que
esa comparación interna se sustituye en memoria por la de v8.
...
import compare_procedural_v8 as v8  # noqa: E402
import qa_preparacion  # noqa: E402


def compare_como_v8(antes, despues, refinamientos):
    """Reemplaza la comparación v7 por la v8 dentro de qa_preparacion.
    ...
    candidato = stage / v8.BASE.name
    report, _ = v8.compare(v8.read_rows(v8.BASE), v8.read_rows(candidato), v8.revisiones())
    if not report['Pasa']:
        raise ValueError('La comparación v8 no pasó')
    return report, []
