"""Construye la entrega v10 = v9 + los siete cortes del lote10, y publica sólo si pasa todo.

Uso: python scripts/preparar_data_v10.py --destino data/releases/continuidad_procedimental_v10
...
GATE = 'scripts/compare_procedural_v10.py'
SALIDAS_EXTRAS = ('comparacion_procedimental_v10.json', 'linaje_cortes_lote10.csv')
...
def main(argv=None):
    ...
    parser.add_argument('--destino', type=Path,
                        default=ROOT / 'data/releases/continuidad_procedimental_v10')
    args = parser.parse_args(argv)
    objetivo = destino_versionado(args.destino)

    cache = ROOT / '.cache'
    cache.mkdir(exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='preparacion-v10-', dir=cache) as temp:
        stage = Path(temp)
        ...
        comandos = [
            ...
            ['scripts/qa_preparacion_v10.py', '--processed', str(stage)],
        ]
    ...
    comparacion = json.loads((objetivo / 'comparacion_procedimental_v10.json')
