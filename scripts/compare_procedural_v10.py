"""Gate v10: los siete cortes del lote10 sobre la entrega procedimental v9.

El esperado se deriva de la entrega v9 firmada y del registro lote10, NO del
...
filas. v9 agregó dos y v10 agrega una más, así que cada versión necesita su propio
gate en vez de reescribir el anterior, que está fijado por hash en la procedencia
de su entrega.
...
3. En los cinco padres revisados no se inventa ni se pierde texto: la
   concatenación de los segmentos es idéntica a la de v9.
...
BASE = ROOT / 'data/releases/continuidad_procedimental_v9/consolidado_base_referencia_final.xlsx'
BASE_SHA = '32752bd52ce5d3b50b0732780154aaef23482908f6671f92a327509e75edf452'
LOTE10 = ROOT / 'data/curation/revisiones_hablantes_lote10.json'
PADRES = (657, 1564, 1706, 1924, 1925, 1995, 2960)
FILAS_BASE = 9725
...
def revisar_corte(padre, antes, despues, entrada, texto_padre):
    ...
    # Ni v9 ni v10 pueden inventar o perder texto respecto de la fuente. Se compara sin
    # espacios porque el constructor recorta el que separaba los tramos al partir.
    for etiqueta, texto in (('v9', texto_antes), ('v10', texto_despues)):
...
def compare(before, after, revisiones):
    ...
    if len(after) != FILAS_BASE + 1:
        raise ValueError(f'v10 debe tener exactamente una fila más que v9: '
                         f'{len(after)} vs {FILAS_BASE} + 1. De los dos cortes nuevos sólo el '
                         f'del padre 1924 agrega una fila; el del 1925 desplaza una frontera '
                         f'que el constructor ya trazaba, sin cambiar el número de segmentos.')
    ...
    # El ID es posicional. Con dos filas nuevas el corrimiento ya no es +1 fijo:
    # vale 0 antes del primer corte, +1 entre los dos cortes y +2 después del
    # segundo. No se fija a mano: se calcula cuántas filas nuevas preceden a cada
    # fila y se exige exactamente ese desplazamiento, ni uno más ni uno menos.
    id_base = {f['ID_Intervencion']: f['ID'] for f in before}
    nuevas = [f['ID_Intervencion'] for f in after if f['ID_Intervencion'] not in id_base]
    if len(nuevas) != 1:
        raise ValueError(f'Se esperaba 1 fila nueva y hay {len(nuevas)}: {nuevas}')
    if [f['ID'] for f in after] != list(range(1, len(after) + 1)):
        raise ValueError('El ID dejó de ser una secuencia contigua desde 1')
    ids_nuevos = sorted(f['ID'] for f in after if f['ID_Intervencion'] not in id_base)
    corridas = []
    for fila in after:
        viejo = id_base.get(fila['ID_Intervencion'])
        if viejo is None:
            continue
        esperado = viejo + sum(1 for i in ids_nuevos if i <= fila['ID'])
        if fila['ID'] != esperado:
            raise ValueError(f'{fila["ID_Intervencion"]}: ID pasó de {viejo} a {fila["ID"]}, '
                             f'se esperaba {esperado} (viejo + {esperado - viejo} filas nuevas '
                             'insertadas antes)')
        if fila['ID'] != viejo:
            corridas.append(fila['ID_Intervencion'])
    # El corrimiento tiene que ser un sufijo por cada punto de inserción: ninguna
    # fila anterior a una inserción puede haberse corrido.
    for fila in after:
        viejo = id_base.get(fila['ID_Intervencion'])
        if viejo is not None and fila['ID'] == viejo and fila['ID'] > min(ids_nuevos):
            raise ValueError(f'{fila["ID_Intervencion"]} conserva su ID aunque hay una fila '
                             'nueva antes: el corrimiento no es un sufijo')
    ...
    if len(extras) != 1:
        raise ValueError(f'se esperaba 1 grupo nuevo y hay {len(extras)}: {extras}')
    ...
    return dict(Perfil='procedimental-v10', Baseline='procedimental-v9', Pasa=True,
...
def revisiones():
    """Las siete revisiones del lote10, indexadas por padre."""
...
def main():
    ...
    if hashlib.sha256(BASE.read_bytes()).hexdigest() != BASE_SHA:
        raise ValueError('La entrega procedimental v9 cambió')
    ...
    (args.candidate / 'comparacion_procedimental_v10.json').write_text(
