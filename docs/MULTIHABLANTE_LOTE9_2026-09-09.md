...
| 278–286 | 2005-02-10 | 82 | **sesión cerrada 82/82** · 12 actores · 94.860 caracteres · **cero cortes**: cola 0, 2c 0 y 2b dio 1 caso falso (`79:1`, De Gregorio es objeto de «en relación al comentario del», no hablante) · la nueva **sección 2d** de `analisis_sesion.py` —calibración inversa sobre los 2.505 segmentos de continuación de la base— propuso 4 lecturas obligatorias y **las 4 resultaron falsas** (`103:1` Marcel sí es Ministro de Hacienda, `119:1` Corbo sí es Presidente, `109:1` el actor es Pablo García, `120:1` es un «por su parte» contrastivo del mismo hablante) · **25 operaciones de OCR en 25 filas** (18 × `cambíanos`→`cambiarios` en 16 sesiones, más `regia`, `Marcell`, `preveen`, `paran`, `S ociedad`, `elim inando`, `estim adas`) y **marca 31** (paréntesis huérfano en `122:1`) · **hallazgo transversal**: dos entradas de `Correcciones` con el mismo `ID_Intervencion` hacen que la segunda pise a la primera — el aplicador parte del virgen por entrada y guarda por ID, así que se perdieron 5 operaciones sin que `--validar` se quejara; era el único ID duplicado de las 1.792 entradas y lo creé esta sesión · tres tests nuevos, uno de invariante que revisa las 3.085 operaciones (criterio §72) |

| 287–295 | 2014-04-17 | 86 | **sesión cerrada 86/86** (80 nuevas) · 16 actores · 98.010 caracteres · **cero cortes**: la cola y 2b dieron 1 caso cada una y ambos son falsos (`6135:1` habla solo Fuentes; en `6133:1` Herrera y Soto son **objetos de un agradecimiento**, no hablantes), 2c en cero y las **6** lecturas obligatorias de 2d también falsas — en `6171:1` «El Ministro de Hacienda señala que» es el propio Alberto Arenas de Mesa, que **es** el Ministro y el `Actor_Final` de la fila; en `6119:1`/`6123:1` los «por su parte» y «en tanto que» comparan **variables** («las volatilidades», «el precio del petróleo Brent»), no personas · **9 operaciones nuevas** · **HALLAZGO GRAVE**: el barrido de hapax propuso `presupuestaría` y el censo contra el **virgen** mostró que el daño no venía del OCR sino del propio registro — **32 operaciones** de una regla automática de tilde le **agregaban** acento a palabras ya correctas (`presupuestaria`→`presupuestaría`, `exposiciones`→`exposiciónes`), todas con la misma justificación plantilla. Se eliminaron y 24 filas pasaron a `Revisiones_Sin_Correccion` con su `Motivo`. Los pisos bajaron por única vez (1.791/3.085 → 1.771/3.062) (criterio §73) |

| 296–304 | 2013-02-14 | 84 | **sesión cerrada 84/84** (80 nuevas) · 11 actores · 107.187 caracteres · **cero cortes**: los 2 casos de la cola caen sobre padres **ya partidos** (5360 en 2 segmentos, 5367 en 4) y una señal sobre padre ya partido no es corte pendiente (§69); 2b dio 1 caso falso (en `5364:1` Miguel Fuentes es **objeto** de «en relación con lo expresado por»); 2c en cero; las **7** lecturas obligatorias de 2d falsas — en `5393:1` el «por su parte» compara **consumo con inversión**, no personas, y en `5337:1`/`5339:1`/`5361:1`/`5378:1` cada «A continuación» es el propio `Actor_Final` siguiendo su presentación · **1 operación** (`et dato`→`el dato`) y **1 marca** · se **dejó** «a pesar que» (62 vs 115): la brecha de frecuencia sola no hace error a la forma minoritaria · `5364:1` se marcó en vez de corregirse porque la frase agramatical tiene **dos reparaciones atestiguadas** en el corpus (criterio §74) |

| 305–313 | 2011-08-18 | 84 | **sesión cerrada 84/84** (80 nuevas) · 13 actores · 95.299 caracteres · **cero cortes** aunque los 2 casos de la cola traían señal **estricta**: caen sobre padres **ya partidos** (4220 en 3 segmentos, 4233 en 2) — tercera acta seguida en que la cola sólo propone sobre padres ya partidos · 2b dio 2 casos falsos (`4266:1` Claro es **objeto** de «Ante una consulta del Consejero…»; `4279:1` Marshall es objeto de un agradecimiento y «Por su parte, el señor Presidente» es el propio De Gregorio) · 2c en cero · las **8** lecturas de 2d falsas · **64 operaciones en 60 filas**, tres familias transversales: el signo «¡» sustituyendo una «i» o una «l» inicial (44 oc. en 40 filas, censadas contra el **virgen** antes de tocar), «ei» por el artículo «el» (14 en 13) y 2 viñetas «►» del PDF · **2 marcas** en vez de corregir: un dígito dañado en letra y una hora **desplazada** que no se puede reordenar con un reemplazo literal (criterio §75) |

| 314–322 | 2009-10-13 | 82 | **sesión cerrada 82/82** (80 nuevas) · 13 actores · 102.639 caracteres · **cero cortes** con las **tres** redes en cero por primera vez en varias actas (ninguna fila en la cola de 74, 2b 0, 2c 0) · las 7 lecturas de 2d falsas, y dos revelan un patrón nuevo: `2714:1` termina en «A continuación,» y `2715:1` empieza en minúscula «el señor Presidente ofrece la palabra…» — **no es un cambio de hablante sino una misma oración partida entre dos filas por el salto de página**, con el mismo `Actor_Final` a ambos lados (igual `2748:1`/`2749:1`); `2715:1` y `2731:1` son traspasos de la palabra, no intervenciones · los 4 candidatos del detector de acento falsos (`centró` verbo, `cuánto` interrogativo, `periodo` conservado, `limite` subjuntivo) · **2 operaciones** (`maquinabas`→`maquinarias`, `equilibro`→`equilibrio`) y **1 marca** en `2751:1`: «represión» es palabra legítima y la frase es gramatical, pero es hápax con «reversión» 289 veces y lectura forzada, así que se marca en vez de elegir (criterio §76) |

| 323–331 | 2015-12-17 | 81 | **sesión cerrada 81/81** (79 nuevas) · 11 actores · 94.320 caracteres · **cero cortes** en la sesión más limpia hasta aquí: ninguna fila en la cola de 74, 2b 0, 2c 0 y de las seis firmas de 2d sólo «A continuación» dio 1 caso, que es la apertura estándar de Vergara más el traspaso de la palabra a Gianelli (una voz, §68) · los **cuatro** detectores dieron 0 candidatos y los signos sospechosos 0 en todo · **2 operaciones** (`descomprensión`→`descompresión`, `exceptivas`→`expectativas`) y **ninguna marca** · la primera trae la prueba más fuerte del trabajo: la **misma fila**, una oración antes, dice «una descompresión de premios por plazo» y el «dicha» la retoma — la fila se atestigua a sí misma (criterio §77) |
| 332 | 2011-04-12 | 83 | **sesión cerrada 83/83** · 12 actores · 85.960 caracteres · **cero cortes**: los diez «El señor X responde que…» tienen a X == `Actor_Final` de la propia fila (X responde, no habla por otro; el criterio queda fijado — es corte sólo si X es un tercero dentro de la fila de otra persona, como el padre 1706 de §48) · el único neto de la cola, padre `3906`, cae sobre un **padre ya dividido** (`3906:1` De Gregorio / `3906:2` Claro) · los 3 «A continuación» y el «por su parte» son de una sola voz · las 4 firmas de 2d dieron 0 candidatas reales («cambió»/«incrementó» son verbos legítimos) · **19 operaciones en 19 filas** y **2 marcas** · **hallazgo transversal**: la comilla recta simple usada como residuo de maquetación — censo de todo el corpus, 26 `'`, 10 apóstrofes legítimos (`Moody's`, `Poor's`, `Managers'`, `L1oyd's`) y 16 residuos, llevados a **1**, que es `'aquerida` marcado porque la compuerta de vocabulario rechaza «adquirida» (el corpus sólo atestigua «adquirido» y el plegado no cubre flexión de género) · `L1oyd's`→`Lloyd's` se marca: `Lloyd` aparece 0 veces en el corpus, no hay forma canónica atestiguada (criterio §78) |
| 333 | 2009-11-12 | 81 | **sesión cerrada 81/81** · 13 actores · 105.315 caracteres · **cero cortes**, verificados por offset y no a ojo: las fronteras acumuladas de los segmentos ya existentes ubican los cuatro netos de la cola sobre `2765:4` De Gregorio (frontera 3101), `2768:1` De Gregorio (frontera exacta), `2768:3` Marshall (frontera 807) y `2779:2` Soto seguido de `2779:3` Marshall (frontera 222), con 1 a 3 caracteres de imprecisión del propio neto · 2b y 2c dieron 0 · en `2780:1` «el señor Claudio Soto», «el señor Soto» y «el señor Gerente de Análisis Macroeconómico» son **la misma persona y su cargo**, no tres hablantes · **14 operaciones en 6 filas** y **2 marcas** · la convención del encabezado de Acuerdo («Política Monetaria.» con punto) está atestiguada en **30 filas** del corpus · `comenzarla` se marca porque caben **tres** reparaciones atestiguadas (`comenzar`/`comenzara`/`comience`) mientras `recuperarla` sí se corrigió porque el «sería» de la misma oración decide · `ALAR` se marca: leer FLAP como ALAR exige confundir F con A **y** P con R a la vez (criterio §79) |
| 334 | 2012-04-17 | 81 | **sesión cerrada 81/81** · 12 actores · 85.351 caracteres · **cero cortes**: ninguna fila en la cola de 74, 2b y 2c en 0, y las **11 lecturas obligatorias** de 2d leídas completas — en 5 de los 6 «responde/contesta» el que responde es el propio `Actor_Final`; el sexto (`4764:1`) es un hipotético («a quien responde que trabajó una hora») y Andrea Repetto es objeto de «lo ha señalado»; `4734:1` es apertura más traspaso puro donde Lehmann no llega a hablar; `4755:1` es la solicitud de Vergara a Soto, que tampoco habla ahí · un **barrido inverso propio** sobre las 81 filas (todo verbo de habla con sujeto distinto del actor) dio **una** candidata, `4780:1` «El señor Gerente de División Estudios manifiesta», y ese cargo es el del actor Luis Óscar Herrera, atestiguado **244 veces** y atado en la propia fila · **1 operación y ninguna marca nueva**: el «que» sobrante en «el consumo privado y que la inversión fija muestran», donde el corpus muestra que la construcción «que X y que Y» es legítima **19 de 20 veces** porque cada cláusula trae su verbo · **se revierte la operación de §79** sobre `2801:1`: el recuento que la justificaba estaba sesgado porque el regex sólo capturaba una de las dos variantes del código de Acuerdo; medidas las dos, el corpus va 29 con punto contra 65 sin punto, y la causa es el salto de línea del título, no el OCR (criterio §80) |
| 335 | 2006-04-13 | 78 | **sesión cerrada 78/78** · 13 actores · 66.996 caracteres · **cero cortes**, los tres netos de la cola descartados **por offset**: `644@6923` cae a 1 caracter de la frontera ya existente hacia `644:7` (el padre tiene 12 segmentos); `647@1170` cae dentro del único segmento de Velasco y lo que dispara el neto es «**Recuerda que** el Gerente… **señaló**», retrospección; `653@3144` cae 35 ch antes de la frontera correcta Desormeaux/De Gregorio y moverla le metería a De Gregorio el «Al respecto, indica que dado que el» cuyo sujeto es Desormeaux — en estas actas cada turno nuevo abre con «El ⟨cargo⟩ señor ⟨nombre⟩ ⟨verbo⟩» · **`639:3` parece un corte y no lo es**: trae dos atribuciones completas pero la fila es entera de Igal Magendzo, que *es* el Gerente de Análisis Macroeconómico; la segunda atribución es repetición del redactor, y lo que sí falta es texto entre los dos tramos, que va marcado · barrido inverso propio: 17 candidatos, los 17 cargo propio, «Se» impersonal o pasado («el ex Presidente señor Greenspan **señalaba**») · **2 operaciones, 1 revertida y 4 marcas** · la compuerta de vocabulario muerde por segunda vez, ahora por **tiempo verbal**: el corpus atestigua el lema («inclina» 115, «inclinación» 20, «inclinar» 6) pero no «inclinaría»; en §78 fue por género (`adquirida`) · `UScts 40` se marca porque equivocarse ahí es un error de **cien veces** en una cifra · al corregir `653:1` desapareció su residuo de espacio antes de signo, así que el techo baja de **4 a 3** como exige la prueba (criterio §81) |
| 336 | 2009-03-12 | 81 | **sesión cerrada 81/81** · 13 actores · 93.895 caracteres · **cero cortes**, los tres netos por offset: `2392@0` es **frontera exacta** de `2392:1` (Desormeaux, el Vicepresidente) en un padre ya dividido en 3; `2397@874` cae dentro del único segmento de Claro y dispara «el escenario que **planteaba** el Gerente de Análisis Internacional», objeto en pasado; `2430@1874` cae dentro del único segmento de Beltrán de Ramón, que *es* el Gerente de División Operaciones Financieras · las dos lecturas obligatorias son la misma fila `2427:1`: De Gregorio responde (es el propio actor) y luego traspasa la palabra a García, que no llega a hablar · barrido inverso: 12 candidatos, los 12 cargo propio o palabras capturadas como nombre · **15 operaciones en 13 filas y 6 marcas** · las **dos únicas comillas rectas dobles** del corpus están aquí y son huérfanas: borradas, familia en **0** · `lí\/IACEC` no se toca: ya estaba adjudicado por un test como el único caso ambiguo con esa secuencia · **la enmienda de «dicho porcentajes» hubo que revertirla**: la «s» cae dentro del rango de una op existente, se extendió como manda §15, y la compuerta rechazó el resultado porque el ancla existente **empieza a media palabra** y el Despues quedaba en el fragmento «orcentaje» — lección: un ancla que arranca a media palabra inhabilita su propio tramo para arreglos posteriores · el «votación.» de `2433:1` es un fragmento **desplazado**, no un residuo: completa la oración de `2434:1` (criterio §82) |
